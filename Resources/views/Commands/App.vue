  ...
  <div class="w-full antialiased" :class="{'overflow-y-auto': selectedCommand == null, 'overflow-hidden': selectedCommand != null}">
    <div class="px-6 pb-4">
      ...
      <div class="container mx-auto">
        <top-bar @search="onSearch" :home="route('artisan.index')"/>
...
export default {
  ...
  computed: {
    ...
    commands(){
        return this.$page.props.commands;
    }
  },
  methods: {
    reload(){
        this.$inertia.get(route('artisan.index'));
    },
    fetchGroups() {
    ...
    runCommand(command, formData) {
      this.loading = true;
      axios.post(route('artisan.json', command.name), formData).then(response => {
